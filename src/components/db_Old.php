...
class SpryApiDB extends SpryApi
{
	...
	protected function column_quote($string)
	{
		...
		return '`' . (strpos($string, '.') ? $this->prefix : '') . str_replace('.', '`.`', preg_replace('/(^#|\(JSON\)\s*)/', '', $string)) . '`'; // customize
	}
	...
	protected function column_push($columns)
	{
		...
		{
			preg_match('/^COUNT\(([a-zA-Z0-9_\-\.]*)\)\s*\(([a-zA-Z0-9_\-]*)\)/i', $value, $match);

			if (isset($match[ 1 ], $match[ 2 ]))
			{
				array_push($stack, 'COUNT(DISTINCT '.$this->column_quote( $match[ 1 ] ) . ') AS ' . $this->column_quote( $match[ 2 ] ));
			}
			else
			{
		...
		if (is_array($where))
		{
			...
			if (isset($where[ 'ORDER' ]))
			{
				...
				if (is_array($ORDER))
				{
					...
					else
					{
						...
						foreach ($ORDER as $column)
						{
							...
							array_push($stack, '`' . (strpos($order_match[ 1 ], '.') ? $this->prefix : '') . str_replace('.', '`.`', $order_match[ 1 ]) . '`' . (isset($order_match[ 3 ]) ? ' ' . $order_match[ 3 ] : ''));
						}
				...
				else
				{
					...
					$where_clause .= ' ORDER BY `' . (strpos($order_match[ 1 ], '.') ? $this->prefix : '') . str_replace('.', '`.`', $order_match[ 1 ]) . '`' . (isset($order_match[ 3 ]) ? ' ' . $order_match[ 3 ] : '');
				}
			}
	...
	protected function select_context($table, $join, &$columns = null, $where = null, $column_fn = null)
	{
		...
		$table = '`' . $this->prefix . $table . '`'; // customize
		...
		)
		{
			...
			foreach($join as $sub_table => $relation)
			{
				...
				if ($match[ 2 ] != '' && $match[ 3 ] != '')
				{
						...
						else
						{
							...
							foreach ($relation as $key => $value)
							{
								...
								if(strpos($value, '.'))
								{
									...
									$value_table = $this->prefix . $value_split[0];
									$value = $value_split[1];
								}
								else
								{
									$value_table = '';//(isset($match[ 5 ]) ? $match[ 5 ] : $match[ 3 ]);
								}
								...
								$joins[] = (
									strpos($key, '.') > 0 ?
										...
										//'"' . str_replace('.', '"."', $key) . '"' :
										'`' . (strpos($key, '.') && !strpos($key, ').') ? $this->prefix : '') . str_replace('.', '`.`', $key) . '`' :
								...
								( $value_table ? '`' . $value_table . '`.`' . $value . '`' : "'" . $value . "'" );
							}
					...
					$table_join[] = $join_array[ $match[ 2 ] ] . ' JOIN `' . $this->prefix . $match[ 3 ] . '` ' . (isset($match[ 5 ]) ?  'AS `' . $this->prefix . $match[ 5 ] . '` ' : '') . $relation;
				}
			}
	...
	public function error()
	{
		...
		return $this->pdo->errorInfo();  // customize
	}
